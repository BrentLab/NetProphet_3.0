...
python ${p_src_code}code/netprophet2/parse_motif_summary.py \
    -a True \
    -i $MOTIFS_DIR \
    -o $MOTIFS_LIST

python ${p_src_code}code/netprophet2/convert_fire2meme.py \
    -i $MOTIFS_LIST \
    -o $OUTPUT_DIR/motif_inference/motifs_pfm/
printf "DONE\n"
...
then
	score_motifs \
        $REGULATORS \
        $OUTPUT_DIR/motif_inference/motifs_pfm/ \
        $PROMOTERS $OUTPUT_DIR/motif_inference/motifs_score \
        ${OUTPUT_DIR}/motif_inference/motif_scoring.log
else
	${p_src_code}code/netprophet2/score_motifs.sh \
        $REGULATORS \
        $OUTPUT_DIR/motif_inference/motifs_pfm/ \
        $PROMOTERS $OUTPUT_DIR/motif_inference/motifs_score \
        ${OUTPUT_DIR}/motif_inference/motif_scoring.log

...
while getopts “:hmlco::g::f::t:r:a:p:d:n:u:j:o:e:y:” OPTION
do
	case $OPTION in
		h)
			usage
...
then
	...
	sbatch --mail-type=FAIL,END --mail-user=dabid@wustl.edu --dependency=afterany:${jid_prep} -J ${job_name} -o ${p_out_err}_%J.out -e ${p_out_err}_%J.err run_netprophet_parallel_init.sh ${targetExpressionFile} ${regulatorExpressionFile} ${allowedMatrixFile} ${perturbationMatrixFile} ${differentialExpressionMatrixFile} ${microarrayFlag} ${nonGlobalShrinkageFlag} ${lassoAdjMtrFileName} ${combinedModelAdjMtrFileName} ${outputDirectory} ${combinedModelAdjLstFileName} ${regulatorGeneNamesFile} ${targetGeneNamesFile}
